...
기본 주소는 `http://localhost:3000`입니다. Wrangler에 Cloudflare 계정 로그인이 되어 있으면 로컬 개발에서도 `AI` 바인딩을 통해 실제 Workers AI를 호출합니다. `GROQ_API_KEY`가 서버 비밀값으로 설정된 환경에서는 Workers AI 한도·일시 오류 또는 생성된 해석의 품질 보정 때 Groq를 사용합니다. 선택 후보가 이미 확정된 질문은 AI가 일시적으로 실패해도 같은 답변 계약을 사용하는 로컬 해석으로 공개 흐름을 이어 갑니다. 열린 추천을 포함한 AI 해석은 구조가 정상인 응답을 확보했다면 문체나 그래프 축 같은 부가 품질 검사 때문에 502로 폐기하지 않고 마지막 구조화 응답을 사용합니다. 로컬 Workers AI 호출도 계정 사용량에 포함됩니다.
...
AI는 질문마다 `선택`, `추천`, `예/아니오`, `결과`, `비교`, `전망`, `조언`, `설명`, `분석` 중 필요한 답변 형태를 정하고, 결과의 `verdict`에서 그 형태에 맞는 답을 첫 문장으로 반환합니다. 특정 메뉴나 질문별 답을 코드에 저장하지 않습니다. 사용자가 질문에 후보를 직접 제시한 선택·예/아니오·비교 질문만 후보를 보존해 후보별 카드를 배정합니다. “아침 뭐 먹을까?” 같은 열린 추천은 구성 단계의 `candidates`를 빈 배열로 유지하고, 질문에 맞는 역할 카드를 공개한 뒤 AI가 카드 근거를 종합해 구체적인 답 하나를 생성합니다. 식사 추천은 카드 의미를 식사량·맛의 강도·온도·질감·조리 방식 같은 프로필로 번역한 뒤 메뉴를 정하며, 같은 브라우저의 최근 추천은 전체 카드가 강하게 지지하지 않는 한 반복하지 않습니다. “강화에 성공할까?” 같은 결과 질문은 성공·실패 후보 카드를 미리 만들지 않고 질문별 역할 카드를 구성한 뒤 카드가 가리키는 한쪽을 직접 말합니다. 취향·맛·포만감·영양·몸 상태·감정·미래 흐름도 카드 원뜻과 자리 역할에 근거해 자유롭게 추론합니다.
