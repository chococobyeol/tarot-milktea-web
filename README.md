...
AI는 질문마다 `선택`, `추천`, `예/아니오`, `비교`, `전망`, `조언`, `설명`, `분석` 중 필요한 답변 형태를 정하고, 결과의 `verdict`에서 그 형태에 맞는 답을 첫 문장으로 반환합니다. 특정 메뉴나 질문별 답을 코드에 저장하지 않습니다. 사용자가 질문에 후보를 직접 제시한 선택·예/아니오·비교 질문만 후보를 보존해 후보별 카드를 배정합니다. “아침 뭐 먹을까?” 같은 열린 추천은 구성 단계의 `candidates`를 빈 배열로 유지하고, 질문에 맞는 역할 카드를 공개한 뒤 AI가 카드 근거를 종합해 구체적인 답 하나를 생성합니다.
