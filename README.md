...
정적 화면과 같은 출처의 API를 하나의 Cloudflare Worker로 배포했습니다. 질문 구성과 카드 해석은 Workers AI를 우선 사용하고, 일일 무료 한도 소진·일시적인 공급자 오류·응답 구조 보정이 필요할 때 Groq의 GPT-OSS 120B로 전환합니다. 보정도 더 작은 모델로 낮추지 않고 같은 모델을 유지합니다. 익명 세션의 사용량 카운터는 D1을 사용합니다.
...
AI가 60초 안에 끝나지 않아도 요청을 바로 끊지 않습니다. 브라우저는 카드 구성 120초, 카드 해석 180초까지 기다리고, 서버는 각각 110초·170초 안에서 공급자 전환과 구조 보정을 수행하되 응답 직렬화용 5초를 별도로 남깁니다. 30초와 90초에는 요청이 계속 진행 중임을 대기 화면에 표시합니다. 공급자별 제한 시간과 전체 마감 시각을 함께 사용하므로 브라우저가 먼저 요청을 닫은 뒤 서버가 불필요한 재시도를 이어가지 않습니다.
...
AI는 질문 전체와 앞선 대화 문맥을 읽고 `선택`, `추천`, `예/아니오`, `결과`, `비교`, `전망`, `조언`, `설명`, `분석` 중 답변 계약과 1~5개의 자리 역할을 정합니다. 서버는 `positions.length`에서 `cardCount`를, 보존한 `answerContract.kind`에서 `verdict.kind`를 기계적으로 파생할 뿐 질문 키워드나 분야별 규칙으로 의미를 다시 판정하지 않습니다. 사용자가 직접 제시한 후보만 질문 또는 앞선 계약에 실제로 존재하는지 확인하고, 열린 추천은 카드 공개 뒤 AI가 구체적인 답 하나를 처음 생성합니다. 카드 ID·정역방향·자리·후보 출처·배열 길이·점수 범위 같은 구조만 서버가 검증하며, 질문에 맞는 의미 연결과 자연스러운 문장은 AI가 작성한 그대로 사용합니다.
