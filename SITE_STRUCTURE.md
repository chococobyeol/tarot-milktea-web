...
- AI 추론: Cloudflare Workers AI 우선, 한도·일시 오류·품질 보정 시 Groq
...
| 구간 | 카드 구성 | 카드 해석 |
|---|---:|---:|
| 브라우저 요청 제한 | 120초 | 180초 |
| 서버 전체 요청 예산 | 110초 | 170초 |
| AI 처리 예산 | 최대 105초 | 최대 165초 |
| Workers AI 1회 제한 (Groq 사용 가능 시) | 30초 | 60초 |
| Groq 1회 제한 | 40초 | 50초 |

서버는 요청 파싱과 D1 확인에 이미 쓴 시간을 AI 처리 예산에서 빼고, 최소 5초를 후속 카운터 갱신과 응답 직렬화에 남긴다. 전체 upstream 호출은 최대 세 번이다. Groq가 연결된 경우 Workers AI 한 번 뒤 Groq와 Groq 보정을 각각 한 번까지 실행한다. Groq가 없을 때는 취소할 수 없는 Workers AI 호출을 겹쳐 실행하지 않고, 한 호출이 남은 AI 마감 시각까지 완료되기를 기다린다. 구조적으로 사용할 수 있는 해석을 이미 받은 경우에는 한 번의 보정 실패 뒤 그 응답을 반환한다.

브라우저는 요청마다 임의 UUID를 `x-tarot-request-id`로 보내고 서버도 같은 값을 응답 헤더와 운영 로그에 사용한다. 로그에는 단계, 공급자, 시도 횟수, 소요 시간과 고정 오류 분류만 기록하며 질문·프롬프트·AI 응답 원문은 기록하지 않는다. 대기 화면은 30초와 90초에 진행 문구를 갱신한다. 서버 마감은 `504 / AI_RESPONSE_TIMEOUT`, 브라우저 자체 마감은 구성·해석별 오류로 구분하고 사용자가 입력한 질문과 선택 카드는 유지한다.
...
| Groq | Workers AI 한도·일시 오류 또는 품질 보정 시 질문과 카드 문맥을 요청 처리 중 사용 | 기본 추론 요청은 미보관. 신뢰성·악용 조사 시 미국 GCP에 최대 30일 임시 보관 가능하며 계정에서 ZDR 설정 가능 |
...
src/
  ...
  lib/tarot.ts               덱, 카드 데이터와 공용 리딩 타입
  lib/reading-quality.ts     AI 응답의 범용 구조·참조 무결성 검사
...
| `GROQ_API_KEY` | Workers AI 한도·일시 오류와 품질 보정 시 사용하는 암호화된 서버 비밀값 |
