...
해석도 Workers AI를 우선 사용한다. Workers AI 한도·일시 오류가 발생하거나 첫 해석이 의미 품질 검사를 통과하지 못하면 Groq의 `openai/gpt-oss-120b`로 전환한다. Groq 첫 응답에는 Strict JSON Schema를 적용하고, 보정도 더 작은 모델로 내리지 않고 같은 120B를 JSON Object Mode로 사용한다. 해석 출력 상한은 3,600토큰이며 최대 세 번의 생성 중 마지막으로 확보한 구조적으로 유효한 응답은 문체·축 이름 같은 부가 품질 검사 때문에 502로 폐기하지 않는다. 카드 ID·자리·방향·원뜻·근거와 신호 합계는 서버가 정렬·주입·정규화한다.
